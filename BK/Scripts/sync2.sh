...
## Description
## Script is generating html from markdown in dirs named starting with book
## scp is used for copying to remote server
## Python script bsync is used for bidirectional synchronizing to server
## I added another for while loop for an archived directory: $src_dir_archive
...
## Traversing through current working books dir
...
## Traversing through archived books dir
find "$src_dir_archive" -mindepth 1 -maxdepth 1 -type d -name 'book*'|
while read D;
	do cd $D;
		$gouda;
done

## Syncing again with server, for changes from $gouda
## colored output
